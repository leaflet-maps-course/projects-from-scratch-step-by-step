...
import { marker, Map, polyline } from "leaflet";

const mapLayout = initializeMap();
// 1.- Abrir el contenido del fichero y enviarlo como string
axios
  .get(
    "https://raw.githubusercontent.com/leaflet-maps-course/resources/main/tracks/wikiloc-quito-ecuador.gpx"
  )
  .then((result) => addPoints(result.data));
  //addPoints(result.data));
...
function drawWaypoints(wayPoints: Array<{lat: number, lon: number}>) {
  wayPoints.map((waypoint) => {
    marker([waypoint.lat, waypoint.lon]).bindPopup(
      `<img src="https://cdn0.iconfinder.com/data/icons/coronavirus-33/512/world-map-point-global-spread-coronavirus-covid-19-512.png" width="100px" />`
    ).addTo(mapLayout);
  })
}
